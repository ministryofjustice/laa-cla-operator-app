...
import { configureAxiosInstance, handleApiCall } from '#src/services/api/baseApiService.js';
...
describe('baseApiService', () => {
	...
	describe('handleApiCall', () => {
		...
		it('wraps failures with a user-facing message and preserves cause', async () => {
			...
			await assert.rejects(
				() => handleApiCall(async () => Promise.reject(originalError), 'loading cases'),
				(error: unknown) => {
					assert(error instanceof Error);
					assert.strictEqual(error.message, 'An unexpected error occurred. Please try again.');
	...
	describe('configureAxiosInstance', () => {
		function createWrapper(baseURL = 'http://existing.local'): AxiosInstanceWrapper {
			return {
				axiosInstance: {
					defaults: {
						baseURL,
		...
		it('sets API base URL and JSON headers', () => {
			...
			assert.strictEqual(wrapper.axiosInstance.defaults.baseURL, 'https://backend.example');
			assert.strictEqual(wrapper.axiosInstance.defaults.headers.common['Content-Type'], 'application/json');
			assert.strictEqual(wrapper.axiosInstance.defaults.headers.common.Accept, 'application/json');
		});
		...
		it('keeps existing base URL when config baseUrl is not a string', () => {
			...
			assert.strictEqual(wrapper.axiosInstance.defaults.baseURL, 'http://keep.local');
			assert.strictEqual(wrapper.axiosInstance.defaults.headers.common['Content-Type'], 'application/json');
			assert.strictEqual(wrapper.axiosInstance.defaults.headers.common.Accept, 'application/json');
		});
	});
