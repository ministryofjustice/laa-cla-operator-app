...
describe('apiInterceptors', () => {
  ...
  it('addLoggingInterceptors registers request and response interceptors', () => {
    const { wrapper, requestUse, responseUse } = createWrapperWithInterceptorStubs();
  ...
  it('addAuthServiceInterceptors sets Authorization header from authService', async () => {
    ...
    const requestFulfilled = requestUse.firstCall.args[0] as (config: { headers: Record<string, string> }) => Promise<{ headers: Record<string, string> }>;
  ...
  it('addAuthServiceInterceptors logs when auth header is added and logging is enabled', async () => {
    ...
    assert.equal(
      consoleLogStub.calledWithMatch('Added JWT authorization header to API request'),
      true
    );
  });
  ...
  it('addAuthServiceInterceptors handles getAuthHeader failures without throwing', async () => {
    ...
    assert.equal(consoleErrorStub.calledWithMatch('Failed to add JWT authorization header: token fetch failed'), true);
  });
  ...
  it('addAuthServiceInterceptors clears tokens on 401 responses', async () => {
    ...
    const responseRejected = responseUse.firstCall.args[1] as (error: unknown) => Promise<never>;
  ...
  it('addAuthServiceInterceptors logs and clears tokens on 401 when logging enabled', async () => {
    ...
    assert.equal(authService.clearTokens.calledOnce, true);
    assert.equal(
      consoleErrorStub.calledWithMatch('API returned 401 Unauthorized - clearing cached tokens'),
      true
    );
  });
  ...
  it('addSessionSilasTokenInterceptor sets bearer token on request config', () => {
    ...
    const requestFulfilled = requestUse.firstCall.args[0] as (config: { headers: Record<string, string> }) => { headers: Record<string, string> };
    const config = { headers: {} };
  ...
  it('addSessionSilasTokenInterceptor logs when logging is enabled', () => {
    ...
    assert.equal(consoleLogStub.calledWithMatch('Added SILAS bearer token to API request'), true);
  });
  ...
  it('addLoggingInterceptors logs request and response details', async () => {
    ...
    requestFulfilled({ method: 'get', baseURL: 'http://api.test', url: '/cases' });
    responseFulfilled({ status: 200, config: { method: 'get', url: '/cases' } });
  ...
  it('addLoggingInterceptors wraps request interceptor errors into Error', async () => {
    ...
    const requestRejected = requestUse.firstCall.args[1] as (error: unknown) => Promise<never>;
    ...
    assert.equal(consoleErrorStub.calledWithMatch('API Request Error: request failed'), true);
  });
