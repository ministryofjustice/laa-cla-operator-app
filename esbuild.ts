...
const copyAssets = async (): Promise<void> => {
  try {
    ...
    const rebrandSourcePath = path.resolve(
      "./node_modules/govuk-frontend/dist/govuk/assets/rebrand",
    );
    if (await fs.pathExists(rebrandSourcePath)) {
      await fs.copy(rebrandSourcePath, path.resolve("./public/assets/rebrand"));
    }
    ...
    plugins: [
      sassPlugin({
        ...
        transform: (source: string): string =>
          source
            .replace(
              /url\(["']?\/assets\/fonts\/(?<fontPath>[^"')]+)["']?\)/gu,
              'url("/assets/fonts/$<fontPath>")',
            )
            .replace(
              /url\(["']?\/assets\/images\/(?<imagePath>[^"')]+)["']?\)/gu,
              'url("/assets/images/$<imagePath>")',
            ),
