...
ENVIRONMENT=$1
HELM_DIR=./helm_deploy/laa-cla-operator-app
APP_NAME=laa-cla-operator-app
...
# Set the deployment host, this will add the prefix of the branch name e.g el-257-deploy-with-circleci or just main
  RELEASE_HOST="$BRANCH_RELEASE_NAME-$APP_NAME.cloud-platform.service.justice.gov.uk"
# Set the ingress name, needs release name, namespace and -green suffix
  IDENTIFIER="$BRANCH_RELEASE_NAME-$K8S_NAMESPACE-green"
  echo "Deploying commit: $GITHUB_SHA under release name: '$BRANCH_RELEASE_NAME'..."

  helm upgrade "$BRANCH_RELEASE_NAME" "$HELM_DIR" \
                --install --wait \
                --namespace="${K8S_NAMESPACE}" \
                --values "$HELM_DIR"/values/values-"$ENVIRONMENT".yaml \
...
deploy_main() {  
  helm upgrade "$APP_NAME" "$HELM_DIR" \
                          --install --wait \
                          --namespace="${K8S_NAMESPACE}" \
                          --values "$HELM_DIR"/values/values-"$ENVIRONMENT".yaml \
                          --set image.repository="$REGISTRY/$REPOSITORY" \
