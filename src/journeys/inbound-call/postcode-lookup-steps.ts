...
export const myEffects = new EffectRegistry()
...
export const MyEffects = {
    ...
    saveFormData: myEffects.register((deps) => (context) => {
        const session = context.getSession()
        if(!session.forms) {
            session.forms = {}
        }
        session.forms.addressLookup = { postcode: context.getPostData("postcode") }
        session.save()
    })
}
...
export const addressLookupStep1 = step({
    ...
    onSubmission: [
        submit({
            validate: true,
            onValid: {
                effects: [MyEffects.saveFormData()],
...
export const addressLookupStep2 = step({
    ...
    title: "Search client's address",
    reachability: { entryWhen: Session("forms.addressLookup.postcode").match(Condition.IsRequired())},
    onAccess: [
        access({
            effects: [MyEffects.postcodeLookup()]
        })
    ],
    blocks: [
        GovUKHeading({
